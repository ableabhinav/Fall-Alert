...
# 3) Video input path
# 4) Execute button
# USAGE python yolo_test.py --input test.mp4 --output test_output.mp4 --yolo yolo-coco
...
# # Authentication 
# s.login("SENDER.EMAIL@gmail.com", "PASSWORD") 
	...
	# boxes, idxs, idArray
		...
		# extract the bounding box coordinates
			...
			# draw a bounding box rectangle and label on the frame
				...
				cv2.imwrite("output-test\\Alert{}.jpg".format(fno), frame)
				# sending the mail 
				# msg += 1
				# if msg==2:
					# s.sendmail("SENDER.EMAIL@gmail.com", "RECEIVER.EMAIL@gmail.com", message)
				print("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
				...
				cv2.putText(frame, text, (x, y - 5),
				cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
				cv2.imwrite("output-test\\Frame{}.jpg".format(fno), frame)
